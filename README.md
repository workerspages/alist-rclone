...
| `INSTANCE_ID` | - | ⚠️ | **[多实例部署必填]** 实例唯一标识符。多个平台共享同一 S3 存储桶时必须设置（如 `northflank`），否则数据会互相覆盖 |
...
> **多实例部署警告**：如果您在多个 PaaS 平台上部署了相同的服务，并且它们指向同一个 S3 存储桶的同一路径，各实例会互相覆盖数据。**解决方案**：为每个实例设置不同的 `INSTANCE_ID`（如 `INSTANCE_ID=northflank`），系统会自动隔离到独立子路径。
